...
then 
  ...
  multipass launch --cpus 4 --memory 7G --disk 50G --name nameless --cloud-init cloud-init.yaml
fi

echo -e "\n\033[1;32m==== Transferring files to VM ====\033[0m\n"
rsync -av -e "ssh -o StrictHostKeyChecking=no -i ./id_ed25519" --delete --exclude={'id_ed25519','id_ed25519.pub','cloud-init.yaml','README.md'} $(pwd) $USER@$(multipass info nameless | grep IPv4 | awk '{ print $2 }'):/home/$USER
